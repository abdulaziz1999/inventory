...
class Laporan_opname extends CI_Controller{
    ...
    function index(){
        ...
        $this->template->load('template', 'laporan/laporan_opname_cutoff',$data);
    }
    ...
    function ajax2($idc, $u=false, $k=false){
        ...
        $get =	$this->db->select('*')->group_by('tb_barang.id_barang')->get('tb_barang');
        ...
        foreach($get->result() as $row){
            ...
            $jml_fisik      = $row->jml_baik + $row->jml_rusak;
            $selisih        = $row->stok - $jml_fisik;
            $jmlhrgrasio    = $selisih * $row->harga_beli;
